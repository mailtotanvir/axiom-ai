...
const guardrailsOptionsSchema = z.object({
  enabled: z.boolean().default(true),
  piiMode: z.enum(["redact", "block", "disabled"]).default("redact"),
...
export const gatewayConfigSchema = baseConfigSchema
  ...
  .extend({
    ...
    /**
     * Guardrails configuration (PII + Prompt Injection):
     * {"enabled":true,"piiMode":"redact","policyEnabled":true,"bannedKeywords":[]}
     */
    GATEWAY_GUARDRAILS: z
      .string()
      .optional()
      .transform((raw, ctx) => {
        if (raw === undefined || raw.trim() === "") {
        ...
        const parsed = guardrailsOptionsSchema.safeParse(jsonValue);
        if (!parsed.success) {
          for (const issue of parsed.error.issues) {
            ctx.addIssue({ code: "custom", message: `GATEWAY_GUARDRAILS: ${issue.message}` });
          }
          return z.NEVER;
        }
        return parsed.data;
...
>;
export type GuardrailsOptionsConfig = NonNullable<
  z.infer<typeof gatewayConfigSchema>["GATEWAY_GUARDRAILS"]
>;
...
export type GatewayConfig = Omit<
  ...
  "GATEWAY_ROUTING" | "GATEWAY_TIER_LIMITS" | "GATEWAY_INPUT_CACHE" | "GATEWAY_GUARDRAILS"
> & {
  GATEWAY_ROUTING: GatewayRouting;
  GATEWAY_TIER_LIMITS: TierLimits;
  GATEWAY_INPUT_CACHE: InputCacheOptionsConfig;
  GATEWAY_GUARDRAILS?: GuardrailsOptionsConfig;
};
...
export function createGatewayConfig(
  ...
  return {
    ...
    GATEWAY_GUARDRAILS: parsed.GATEWAY_GUARDRAILS,
  };
}
